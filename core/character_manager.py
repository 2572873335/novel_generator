...
class CharacterManager:
    ...
    def create_characters(
        ...
        for char_data in characters_data:
            ...
            if character.name in name_set:
                duplicates.append(character.name)
                print(f"⚠️ [严重警告] 检测到重名角色: '{character.name}'")
                print(f"   该角色将被跳过，请检查人物设定！")
                continue
        ...
        if duplicates:
            print(f"\n❌ [姓名冲突] 以下姓名重复: {', '.join(duplicates)}")
            print(f"   请在人物设定中修正后重新生成！")

        self._save_characters()
        return self.characters

    def check_name_conflicts(self, new_name: str) -> bool:
        """检查新角色名是否与现有角色冲突"""
        if new_name in self.name_index:
            existing = self.name_index[new_name]
            print(f"⚠️ 姓名冲突: '{new_name}' 已存在（角色: {existing.role}）")
            return True
        return False
    ...
    def get_main_characters(self) -> List[Character]:
        """获取主要角色"""
        return [
            ch for ch in self.characters if ch.role in ["protagonist", "antagonist"]
        ]
...
"""
    ...
    def check_character_consistency(
        self, chapter_content: str, characters_involved: List[str]
    ) -> Dict[str, Any]:
        """
        检查角色一致性（简化版本）
        实际可以使用LLM进行更复杂的检查
