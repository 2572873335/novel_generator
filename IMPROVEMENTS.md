...
## 最新更新 (2026-02-22) - 编辑锐评补丁v2

### 基于起点编辑锐评的修复

**编辑指出的后室问题：**
1. 人物命名混乱 - CharacterDesigner/PlotArchitect/正文使用不同名字
2. 设定矛盾 - "无修炼体系"但有"认知深化五阶段"
3. 场景重复 - 锅炉房、办公室多次出现
4. 势力浪费 - M.E.G.建立后未持续出现

**新增修复：**

#### 1. 世界类型支持 (core/writing_constraint_manager.py)
- 新增 `world_type` 字段：xianxia/backrooms/sci-fi
- 新增 `has_cultivation` 字段：是否包含修炼体系
- 后室类型自动应用特殊约束

#### 2. 后室世界观约束
- 禁止修仙词汇（灵气、法力、金丹、元婴等）
- 禁止"认知深化Stage 1-5"作为修仙升级
- 实体规则绝对化（直视笑魇即死，不是掉SAN值）

#### 3. 场景重复检测
- 新增 `visited_scenes` 追踪已访问场景
- 检测章节标题重复（critical级别违规）
- 同一场景不能简单换皮重复

#### 4. 势力持续出现检测
- 新增 `introduced_factions` 追踪已引入势力
- 新增 `faction_chapter_map` 记录势力最后出现章节
- 势力超过5章不出现则警告

#### 5. 自动约束更新
- 章节写作后自动提取标题加入visited_scenes
- 自动检测M.E.G.、B.N.T.G.、基金会等势力
- 势力每次出现更新最后章节记录

---

## 上一更新 (2026-02-22) - 后室世界观测试
