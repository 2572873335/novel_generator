...
class WriterAgent:
    ...
    def write_session(self) -> Dict[str, Any]:
        ...
        print(f"   进度: {progress['completed_chapters']}/{progress['total_chapters']} 章")
        ...
        if review_result['score'] < 7.0:
            ...
            chapter_content = self._revise_chapter(chapter_number, chapter_content, review_result)
        else:
        ...
        self._update_progress(chapter_number, 'completed', word_count, review_result['score'])
    ...
    def _load_writing_context(self, chapter_number: int) -> Dict[str, Any]:
        ...
        # 加载前一章节内容（用于连贯性）
        if chapter_number > 1:
            prev_chapter_file = os.path.join(self.chapters_dir, f'chapter-{chapter_number-1:03d}.md')
            if os.path.exists(prev_chapter_file):
                with open(prev_chapter_file, 'r', encoding='utf-8') as f:
                    # 只加载最后500字作为上下文
                    content = f.read()
                    context['previous_chapter_ending'] = content[-500:] if len(content) > 500 else content
...
请直接输出章节内容，不要添加额外说明。"""
    ...
    def _self_review(self, chapter_number: int, content: str, 
    ...
    def _revise_chapter(self, chapter_number: int, content: str, 
    ...
    def _update_progress(self, chapter_number: int, status: str, 
        ...
        try:
            ...
            # 更新整体进度
            completed = sum(1 for ch in progress['chapters'] if ch['status'] == 'completed')
            progress['completed_chapters'] = completed
            progress['total_word_count'] = sum(ch.get('word_count', 0) for ch in progress['chapters'])
    ...
    def _mock_llm_write(self, prompt: str, chapter_info: Dict) -> str:
        ...
        实际实现中应该调用真实的LLM API
        """
        title = chapter_info.get('title', f'第{chapter_info.get("chapter_number", 1)}章')
        
        # 生成模拟内容
        content = f"""# {title}

这是{title}的内容。在实际实现中，这里将由LLM根据提示生成完整的章节内容。
