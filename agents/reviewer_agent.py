...
class ReviewResult:
    ...
    combat_consistency: float = 8.0  # 战力一致性
    timeline_consistency: float = 8.0  # 时间线一致性
    name_consistency: float = 8.0  # 姓名一致性
...
class ReviewerAgent:
    ...
    def review_chapter(self, chapter_number: int) -> ReviewResult:
        ...
        print("   正在评估战力一致性...")
        combat_score = self._evaluate_combat_consistency(chapter_content, context)

        print("   正在评估时间线一致性...")
        timeline_score = self._evaluate_timeline_consistency(chapter_content, context)

        print("   正在评估姓名一致性...")
        name_score = self._evaluate_name_consistency(chapter_content, context)
    ...
    def _save_review_report(self, result: ReviewResult):
        ...
        report_file = os.path.join(
            reviews_dir, f"review-{result.chapter_number:03d}.md"
        )
...
"""
    ...
    def _evaluate_combat_consistency(
        self, content: str, context: Dict[str, Any]
    ) -> float:
        """评估战力一致性"""
        score = 8.0
        world_rules = context.get("world_rules", {})
        power_rules = world_rules.get("power_rules", {})

        realm_keywords = ["炼气", "筑基", "金丹", "元婴", "化神", "返虚", "合道"]
        realm_mentions = {}
        for realm in realm_keywords:
            matches = re.findall(
                rf"{realm}[一二三四五六七八九十\d]*[层期前后巅峰]*", content
            )
            if matches:
                realm_mentions[realm] = matches

        if "越级" in content or "越境" in content:
            if "代价" not in content and "代价" not in content:
                score -= 1.5

        level_pattern = (
            r"[炼筑金元化返合][气基丹婴神虚道][一二三四五六七八九十\d]*[层期前后巅峰]*"
        )
        levels_found = re.findall(level_pattern, content)
        if len(levels_found) > 3:
            unique_levels = list(set(levels_found))
            if len(unique_levels) < len(levels_found) * 0.5:
                score -= 0.5

        return max(1.0, min(10.0, score))

    def _evaluate_timeline_consistency(
        self, content: str, context: Dict[str, Any]
    ) -> float:
        """评估时间线一致性"""
        score = 8.0

        time_keywords = ["年前", "月前", "日前", "昨天", "前天", "去年", "今年"]
        time_mentions = []
        for kw in time_keywords:
            matches = re.findall(rf"\d+{kw}|\w+{kw}", content)
            time_mentions.extend(matches)

        if len(time_mentions) > 2:
            years_ago = re.findall(r"(\d+)年", content)
            if years_ago:
                try:
                    year_values = [int(y) for y in years_ago]
                    if max(year_values) - min(year_values) > 10:
                        score -= 1.0
                except:
                    pass
        ...
        if len(ages) > 1:
            try:
                age_values = [int(a) for a in ages]
                if max(age_values) - min(age_values) > 30:
                    score -= 0.5
            except:
                pass
    ...
    def _evaluate_name_consistency(
        ...
        for name in known_names:
            if name in content:
                surname_variants = self._find_name_variants(content, name)
                if surname_variants:
                    score -= 2.0
                    break
    ...
    def _find_name_variants(self, content: str, original_name: str) -> List[str]:
        """查找姓名变体"""
        if len(original_name) < 2:
            return []

        surname = original_name[0]
        given_name = original_name[1:] if len(original_name) > 1 else ""

        variants = []
        pattern = rf"{surname}\w{{1,3}}"
        matches = re.findall(pattern, content)

        for match in matches:
            if match != original_name and len(match) >= 2:
                if given_name and given_name[0] in match:
                    pass
                else:
